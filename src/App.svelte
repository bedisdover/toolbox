...
<div class="grid gap-x-8 gap-y-3" style="grid-template-columns: max-content 1fr">
  <div class="label">文件夹</div>
  <div
    class="p-6 flex flex-col justify-center items-center border-2 border-dashed rounded"
    on:dragover|preventDefault|stopPropagation
    on:drop|preventDefault|stopPropagation="{(e) => {
      addDirs(e.dataTransfer.files)
    }}">
    <FileOpener multiple onOpen="{addDirs}" />
    <div class="mt-1.5 text-gray-500">或将文件夹拖放到此处</div>
  ...
  <button
    class="mt-8 col-start-2 justify-self-end bg-sky-500 hover:bg-sky-700 {btnClass}"
    on:click="{submit}"
    on:animationend="{() => {
      btnClass = ''
    }}">立即选择</button>
...
<div
  class="absolute left-0 right-0 bottom-4 flex items-center justify-center text-gray-400 cursor-pointer hover:text-sky-500"
  on:click="{() => {
  ...
  <Icon name="bilibili" class="ml-1.5 mr-0.5 text-lg" />
  关注我
</div>
